...
`Bagging` and `Boosting` algorithms are statistically significant better than the other algorithms. However, there is no clear and only winner which is acceptable because each algorithm has its own advantages and disadvantages that would be useful for different datasets.