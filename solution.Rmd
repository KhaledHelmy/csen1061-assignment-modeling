...
```{r}
getMeasureCombinations <- function(measure_comparison) {
  ...
  for(measure_combination in measure_combinations) {
    ...
    print(measure_data)
    print(t.test(measure_data[[measure_combination[1]]], measure_data[[measure_combination[2]]], paired = TRUE))
  }
}
```
...
```{r}
accuracy_combinations <- getMeasureCombinations(accuracy_comparison)
```

2. Recall

```{r}
recall_combinations <- getMeasureCombinations(recall_comparison)
```

3. Precision

```{r}
precision_combinations <- getMeasureCombinations(precision_comparison)
```

4. F Score

```{r}
f_score_combinations <- getMeasureCombinations(f_score_comparison)
```

5. Mean Error

```{r}
mean_error_combinations <- getMeasureCombinations(mean_error_comparison)
```